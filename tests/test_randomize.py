...
def test_sample_scene_within_bounds():
    ...
    assert all(abs(v) <= dr.light_xy_jitter for v in scene.light_pos_xy)
    assert all(0.1 <= c <= 1.4 for c in scene.light_diffuse)
...
@rendering
def test_occluder_and_lighting_change_the_render():
    ...
    try:
        ...
        apply_scene(world, DomainRandomizer().sample_scene(np.random.default_rng(5), 3))
...
def test_domain_randomization_degrades_a_clean_model():
    ...
    dr = DomainRandomizer()
    clean_tr, clean_val = train_val_split(generate_dataset(n_samples=300, seed=0), seed=0)
    _, rand_val = train_val_split(generate_dataset(n_samples=300, seed=0, randomizer=dr), seed=0)
...
def test_committed_metrics_show_robust_beats_clean():
    ...
    randomization the robust model beats the clean model and clears the baseline; neither regresses on
    clean. Regenerated by `make train`."""
    import json
    ...
    assert sa["robust_model"]["clean"] >= sa["clean_model"]["clean"] - 1e-9        # no clean regression
